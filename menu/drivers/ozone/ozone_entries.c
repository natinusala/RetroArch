      ...
      menu_entry_t *entry)
{
   ...
   if (entry->checked)
   {
      ...
      ozone_draw_icon(video_info, 30, 30, ozone->theme->textures[OZONE_THEME_TEXTURE_CHECK], x - 20, y - 22, 0, 1, ozone->theme_dynamic.entries_checkmark);
   ...
   bool is_playlist)
{
   ...
   for (i = 0; i < entries_end; i++)
   {
      ...
      /* Borders */
      ozone_draw_quad(video_info, x_offset + 456-3, y - 3 + scroll_y, entry_width + 10 - 3 -1, 1, ozone->theme_dynamic.entries_border);
      ozone_draw_quad(video_info, x_offset + 456-3, y - 3 + button_height + scroll_y, entry_width + 10 - 3-1, 1, ozone->theme_dynamic.entries_border);
...
border_iterate:
   ...
   for (i = 0; i < entries_end; i++)
   {
      ...
      if (tex != ozone->icons_textures[OZONE_ENTRIES_ICONS_TEXTURE_SUBSETTING])
      {
         ...
         menu_display_blend_begin(video_info);
         ozone_draw_icon(video_info, 46, 46, texture, x_offset + 451+5+10, y + scroll_y, 0, 1, icon_color);
         menu_display_blend_end(video_info);
      ...
      /* Draw text */
      ozone_draw_text(video_info, ozone, rich_label, text_offset + x_offset + 521, y + 8 - 1 + scroll_y, TEXT_ALIGN_LEFT, &ozone->fonts.entries_label, COLOR_TEXT_ALPHA(ozone->theme->text_rgba, alpha_uint32), false);
      ozone_draw_text(video_info, ozone, sublabel_str, x_offset + 470, y + 80 - 20 - 3 + scroll_y, TEXT_ALIGN_LEFT, &ozone->fonts.entries_sublabel, COLOR_TEXT_ALPHA(ozone->theme->text_sublabel_rgba, alpha_uint32), false);
      ...
      menu_animation_ticker(&ticker);
      ozone_draw_entry_value(ozone, video_info, entry_value_ticker, x_offset + 426 + entry_width, y + 8 - 1 + scroll_y,alpha_uint32, &entry);
